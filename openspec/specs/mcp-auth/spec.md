# Spec: MCP Authentication

## Purpose
Define how every inbound MCP request is authenticated. Authentication uses a two-credential system: an organization-scoped API key (Bearer token) and a per-request user email for auditing. Validation is delegated entirely to the NestJS backend — the MCP server forwards credentials and trusts the backend's decision.
